...
func TestStartCleansSpawnedProcessOnReadinessTimeout(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("uses a POSIX fake dolt process")
	}
	townRoot := t.TempDir()
	setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), "timeout")
	...
	var spawned doltServerIncarnation
	err = start(townRoot, func(incarnation doltServerIncarnation) { spawned = incarnation })
	if err == nil || !strings.Contains(err.Error(), "not accepting connections") {
		t.Fatalf("start error = %v, want readiness timeout", err)
	}
	if spawned.PID <= 0 {
		t.Fatal("start did not publish the spawned process incarnation")
	}
	if processIsAlive(spawned.PID) {
		t.Fatalf("spawned Dolt process %d survived readiness failure", spawned.PID)
	}
	if data, readErr := os.ReadFile(DefaultConfig(townRoot).PidFile); readErr == nil && strings.TrimSpace(string(data)) == strconv.Itoa(spawned.PID) {
		t.Fatalf("PID file still owns failed process %d", spawned.PID)
	}
}
...
func TestClassifyLocalDoltServers(t *testing.T) {
	...
	got := classifyLocalDoltServers(3307, expectedDir, listeners, func(pid int) doltProcessEvidence {
		return evidence[pid]
	})
	...
	want := []DoltServerClass{
		DoltServerCanonical,
		DoltServerCanonical,
		DoltServerConfiguredPortImposter,
		DoltServerOwnedTestLeak,
		DoltServerUnknown,
		DoltServerOwnedTownLeak,
	}
	for i := range want {
		if got[i].Class != want[i] {
			t.Errorf("listener %d class = %q, want %q", i, got[i].Class, want[i])
		}
	}
	if !got[2].Actionable() || !got[3].Actionable() || !got[5].Actionable() {
		t.Fatal("configured-port imposters and positively owned leaks must be actionable")
	}
	if got[4].Actionable() {
		t.Fatal("unknown listeners must remain report-only")
	}
}
...
func TestRemediateInventoryIsDryRunFirstAndFailsIfActionableRemain(t *testing.T) {
	...
	if err := remediateInventory([]LocalDoltServer{actionable, unknown}, false, terminate, nil); err != nil {
		t.Fatalf("dry run: %v", err)
	}
	if len(terminated) != 0 {
		t.Fatalf("dry run terminated PIDs: %v", terminated)
	}

	err := remediateInventory([]LocalDoltServer{actionable, actionable, unknown}, true, terminate, func() []LocalDoltServer {
		return []LocalDoltServer{actionable, unknown}
	})
	if err == nil {
		t.Fatal("apply returned nil while an actionable entry remained")
	}
	if len(terminated) != 1 || terminated[0] != actionable.PID {
		t.Fatalf("terminated PIDs = %v, want only %d", terminated, actionable.PID)
	}
}

func TestRemediateConfiguredInventoryScopesSignalsAndRemainingCheck(t *testing.T) {
	configured := LocalDoltServer{DoltListener: DoltListener{PID: 401}, Class: DoltServerConfiguredPortImposter}
	townLeak := LocalDoltServer{DoltListener: DoltListener{PID: 402}, Class: DoltServerOwnedTownLeak}
	testLeak := LocalDoltServer{DoltListener: DoltListener{PID: 403}, Class: DoltServerOwnedTestLeak}
	terminated := []int{}

	err := remediateConfiguredInventory(
		[]LocalDoltServer{configured, townLeak, testLeak},
		true,
		func(pid int) error { terminated = append(terminated, pid); return nil },
		func() []LocalDoltServer { return []LocalDoltServer{configured, townLeak, testLeak} },
	)
	if err == nil {
		t.Fatal("apply returned nil while configured-port imposter remained")
	}
	if len(terminated) != 1 || terminated[0] != configured.PID {
		t.Fatalf("terminated PIDs = %v, want configured-only PID %d", terminated, configured.PID)
	}
}

func TestRemediateOwnedLeaksSinceRequiresOwnershipAndExcludesBaseline(t *testing.T) {
	baselineLeak := LocalDoltServer{DoltListener: DoltListener{PID: 501}, Class: DoltServerOwnedTownLeak}
	newLeak := LocalDoltServer{DoltListener: DoltListener{PID: 502}, Class: DoltServerOwnedTestLeak}
	unknown := LocalDoltServer{DoltListener: DoltListener{PID: 503}, Class: DoltServerUnknown}
	terminated := []int{}

	err := remediateOwnedLeaksSince(
		[]LocalDoltServer{baselineLeak, newLeak, unknown},
		map[int]bool{baselineLeak.PID: true},
		func(pid int) error { terminated = append(terminated, pid); return nil },
		func() []LocalDoltServer { return []LocalDoltServer{baselineLeak, unknown} },
	)
	if err != nil {
		t.Fatalf("remediateOwnedLeaksSince: %v", err)
	}
	if len(terminated) != 1 || terminated[0] != newLeak.PID {
		t.Fatalf("terminated PIDs = %v, want only new positively owned leak %d", terminated, newLeak.PID)
	}
}

func TestRemediateTestLeaksSinceSignalsOnlyNewTestOwnedListeners(t *testing.T) {
	baseline := LocalDoltServer{
	...
	unknown := LocalDoltServer{DoltListener: DoltListener{PID: 555, Port: 4555}, Class: DoltServerUnknown}
	terminated := []int{}
	rescans := 0
	...
	if len(terminated) != 1 || terminated[0] != newLeak.PID {
		t.Fatalf("terminated PIDs = %v, want only new test-owned leak %d", terminated, newLeak.PID)
	}
}
...
func TestRemediateTestLeaksSinceRejectsChangedOwnerBeforeSignal(t *testing.T) {
	...
	if terminated {
		t.Fatal("cleanup signaled a listener whose ownership changed after selection")
	}
}
...
func TestRemediatePreviewedTestLeaksSignalsOnlyRevalidatedPreview(t *testing.T) {
	...
	configured := LocalDoltServer{DoltListener: DoltListener{PID: 605, Port: 4605}, Class: DoltServerConfiguredPortImposter}
	preview := []TestLeakSelection{newTestLeakSelection(previewed)}
	terminated := []int{}
	...
	if err == nil {
		t.Fatal("apply returned nil while a previewed test leak remained")
	}
	if len(terminated) != 1 || terminated[0] != previewed.PID {
		t.Fatalf("terminated PIDs = %v, want only previewed test leak %d", terminated, previewed.PID)
	}
}
...
func TestRemediatePreviewedTestLeaksDefaultsToNoSignals(t *testing.T) {
	...
	if terminated {
		t.Fatal("preview signaled a test leak")
	}
}
...
func TestMigrateRigFromBeadsUpgradesVersionOneReceipts(t *testing.T) {
	for _, phase := range []databaseMigrationPhase{databaseMigrationPrepared, databaseMigrationStaged, databaseMigrationTargetReady} {
		t.Run(string(phase), func(t *testing.T) {
			t.Setenv("GT_DOLT_PORT", "1")
			townRoot := t.TempDir()
			rigName := "legacy-" + strings.ReplaceAll(string(phase), "-", "_")
			sourcePath := setupDoltDB(t, filepath.Join(townRoot, "legacy"), rigName)
			targetPath := filepath.Join(townRoot, ".dolt-data", rigName)
			if phase == databaseMigrationStaged {
				setupDoltDB(t, targetPath, databaseMigrationStageName)
			}
			if phase == databaseMigrationTargetReady {
				setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
			}
			if err := os.MkdirAll(filepath.Join(townRoot, rigName, "mayor", "rig", ".beads"), 0o755); err != nil {
				t.Fatal(err)
			}
			...
			if err := MigrateRigFromBeads(townRoot, rigName, sourcePath); err != nil {
				t.Fatalf("resume version-one %s migration: %v", phase, err)
			}
			if _, err := os.Stat(databaseMigrationReceiptPath(townRoot, rigName)); !os.IsNotExist(err) {
				t.Fatalf("migration receipt remains after upgrade: %v", err)
			}
			if _, err := os.Stat(sourcePath); !os.IsNotExist(err) {
				t.Fatalf("legacy source remains after upgrade: %v", err)
			}
		})
	}
}

func TestMigrateRigFromBeadsPreservesPartialVersionOneCleanupRemoving(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	...
	if err := MigrateRigFromBeads(townRoot, rigName, sourcePath); err == nil || !strings.Contains(err.Error(), "no durable identity") {
		t.Fatalf("resume partial version-one cleanup error = %v, want durable-identity refusal", err)
	}
	if _, err := os.Stat(cleanupPath); err != nil {
		t.Fatalf("identity-less cleanup was not preserved: %v", err)
	}
	receipt, err := readDatabaseMigrationReceipt(townRoot, databaseMigrationReceiptPath(townRoot, rigName))
	if err != nil {
		t.Fatal(err)
	}
	if receipt.Version != 1 || receipt.Phase != databaseMigrationCleanupRemoving {
		t.Fatalf("legacy receipt mutated on refusal: %#v", receipt)
	}
}
...
func TestMigrateRigFromBeadsResumesInterruptedVersionOneUpgrade(t *testing.T) {
	...
	if err := writeDatabaseMigrationReceipt(databaseMigrationReceiptPath(townRoot, rigName), databaseMigrationReceipt{
		...
		StagePath:     filepath.Join(targetPath, databaseMigrationStageName),
		SourceDigest:  digest,
		TargetToken:   strings.Repeat("a", 64),
		LegacyUpgrade: true,
	...
	if err := MigrateRigFromBeads(townRoot, rigName, sourcePath); err != nil {
		t.Fatalf("resume interrupted version-one upgrade: %v", err)
	}
	if _, err := os.Stat(databaseMigrationReceiptPath(townRoot, rigName)); !os.IsNotExist(err) {
		t.Fatalf("migration receipt remains after resume: %v", err)
	}
...
func TestMigrateRigFromBeadsPreservesClaimWhenTargetChangesBeforeRemoval(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "changed-target"
	sourcePath := filepath.Join(townRoot, "legacy", rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	targetToken := setupDatabaseMigrationTargetClaim(t, targetPath)
	cleanupPath := setupDoltDB(t, filepath.Dir(sourcePath), filepath.Base(sourcePath)+".migration-cleanup-"+targetToken)
	digest, err := databaseMigrationTreeDigest(targetPath)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMigrateRigFromBeadsPreservesReplacementAfterFinalCleanupRevalidation(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "cleanup-final-swap"
	sourcePath := filepath.Join(townRoot, "legacy", rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	targetToken := setupDatabaseMigrationTargetClaim(t, targetPath)
	cleanupPath := setupDoltDB(t, filepath.Dir(sourcePath), filepath.Base(sourcePath)+".migration-cleanup-"+targetToken)
	...
	originalPath := cleanupPath + ".original"
	previous := databaseMigrationAfterCleanupRevalidation
	var once sync.Once
	var hookErr error
	databaseMigrationAfterCleanupRevalidation = func() {
		once.Do(func() {
			if err := os.Rename(cleanupPath, originalPath); err != nil {
				hookErr = err
				return
			}
			if err := os.MkdirAll(cleanupPath, 0o700); err != nil {
	...
	t.Cleanup(func() { databaseMigrationAfterCleanupRevalidation = previous })
	...
	if got, err := os.ReadFile(filepath.Join(cleanupPath, "foreign")); err != nil || string(got) != "preserve" {
		t.Fatalf("replacement cleanup changed: data = %q, err = %v", got, err)
	}
	if _, err := os.Stat(filepath.Join(originalPath, databaseMigrationCleanupClaimName)); err != nil {
		t.Fatalf("original cleanup claim was not preserved: %v", err)
	}
	if _, err := os.Stat(receiptPath); err != nil {
		t.Fatalf("migration receipt was not preserved: %v", err)
	}
}

func TestMigrateRigFromBeadsPreservesReplacementAtCleanupEmptyMove(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "cleanup-empty-swap"
	sourcePath := filepath.Join(townRoot, "legacy", rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	targetToken := setupDatabaseMigrationTargetClaim(t, targetPath)
	...
	if err := writeDatabaseMigrationReceipt(receiptPath, databaseMigrationReceipt{
		...
		CleanupToken: cleanupToken,
		TargetToken:  targetToken,
		Phase:        databaseMigrationCleanupEmpty,
	}); err != nil {
		t.Fatal(err)
	}
	originalPath := cleanupPath + ".original"
	previous := databaseMigrationBeforeCleanupEmptyMove
	var once sync.Once
	var hookErr error
	...
	t.Cleanup(func() { databaseMigrationBeforeCleanupEmptyMove = previous })
	...
	if info, err := os.Stat(originalPath); err != nil || !info.IsDir() {
		t.Fatalf("original cleanup directory was not preserved: %v", err)
	}
	if _, err := os.Stat(receiptPath); err != nil {
		t.Fatalf("migration receipt was not preserved: %v", err)
	}
...
func TestMigrateRigFromBeadsResumesCleanupEmptyAfterClaimRemoval(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "cleanup-empty-resume"
	sourcePath := filepath.Join(townRoot, "legacy", rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	targetToken := setupDatabaseMigrationTargetClaim(t, targetPath)
	cleanupPath := sourcePath + ".migration-cleanup-" + targetToken
	...
	if err := MigrateRigFromBeads(townRoot, rigName, sourcePath); err != nil {
		t.Fatalf("resume cleanup-empty migration: %v", err)
	}
	if _, err := os.Stat(cleanupPath); !os.IsNotExist(err) {
		t.Fatalf("cleanup path remains after resume: %v", err)
	}
	emptyPath := filepath.Join(townRoot, ".runtime", "dolt-database-migrations", "empty", canonicalDatabaseName(rigName)+"-"+strings.Repeat("a", 64))
	entries, err := os.ReadDir(emptyPath)
	if err != nil {
		t.Fatalf("reading retained identity-bound empty quarantine: %v", err)
	}
	if len(entries) != 0 {
		t.Fatalf("retained empty quarantine contains data: %v", entries)
	}
}

func TestMigrateRigFromBeadsPreservesUnclaimedCleanupEmptyData(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "cleanup-empty-foreign"
	sourcePath := filepath.Join(townRoot, "legacy", rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	targetToken := setupDatabaseMigrationTargetClaim(t, targetPath)
	cleanupPath := sourcePath + ".migration-cleanup-" + targetToken
	...
	if err := os.WriteFile(filepath.Join(cleanupPath, "foreign"), []byte("preserve"), 0o600); err != nil {
		t.Fatal(err)
	}
	digest, err := databaseMigrationTreeDigest(targetPath)
	if err != nil {
	...
	err = MigrateRigFromBeads(townRoot, rigName, sourcePath)
	if err == nil || !strings.Contains(err.Error(), "unclaimed entry") {
		t.Fatalf("migration error = %v, want unclaimed-data refusal", err)
	}
	if got, err := os.ReadFile(filepath.Join(cleanupPath, "foreign")); err != nil || string(got) != "preserve" {
		t.Fatalf("unclaimed cleanup data changed: data = %q, err = %v", got, err)
	}
}

func TestMigrateRigFromBeadsRollsBackReplacementMovedAfterSourceValidation(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "source-rename-swap"
	sourcePath := setupDoltDB(t, filepath.Join(townRoot, "legacy"), rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	digest, err := databaseMigrationTreeDigest(sourcePath)
	if err != nil {
	...
	originalPath := sourcePath + ".original"
	previous := databaseMigrationAfterSourceClaimValidation
	var once sync.Once
	var hookErr error
	databaseMigrationAfterSourceClaimValidation = func() {
		once.Do(func() {
			...
			hookErr = os.WriteFile(filepath.Join(sourcePath, "foreign"), []byte("preserve"), 0o600)
		})
	}
	t.Cleanup(func() { databaseMigrationAfterSourceClaimValidation = previous })
	...
	if err == nil || !strings.Contains(err.Error(), "source identity changed") {
		t.Fatalf("migration error = %v, want source-identity refusal", err)
	}
	if got, err := os.ReadFile(filepath.Join(sourcePath, "foreign")); err != nil || string(got) != "preserve" {
		t.Fatalf("replacement source was not rolled back: data = %q, err = %v", got, err)
	}
	if _, err := os.Stat(filepath.Join(originalPath, databaseMigrationCleanupClaimName)); err != nil {
		t.Fatalf("original source claim was not preserved: %v", err)
	}
	if _, err := os.Stat(receiptPath); err != nil {
		t.Fatalf("migration receipt was not preserved: %v", err)
	}
...
func TestMigrateRigFromBeadsResumesAfterPartialCleanupRemoval(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	rigName := "partial-cleanup"
	sourcePath := filepath.Join(townRoot, "legacy", rigName)
	targetPath := setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), rigName)
	targetToken := setupDatabaseMigrationTargetClaim(t, targetPath)
	cleanupPath := setupDoltDB(t, filepath.Dir(sourcePath), filepath.Base(sourcePath)+".migration-cleanup-"+targetToken)
	if err := os.WriteFile(filepath.Join(cleanupPath, "extra"), []byte("remove"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(targetPath, "extra"), []byte("remove"), 0o600); err != nil {
		t.Fatal(err)
	}
	digest, err := databaseMigrationTreeDigest(targetPath)
	if err != nil {
	...
	previous := databaseMigrationBeforeCleanupChildRemoval
	removals := 0
	databaseMigrationBeforeCleanupChildRemoval = func(string) error {
		removals++
		if removals == 2 {
			return errors.New("injected partial cleanup")
		}
		return nil
	}
	t.Cleanup(func() { databaseMigrationBeforeCleanupChildRemoval = previous })

	err = MigrateRigFromBeads(townRoot, rigName, sourcePath)
	if err == nil || !strings.Contains(err.Error(), "injected partial cleanup") {
		t.Fatalf("first migration error = %v, want injected partial cleanup", err)
	}
	if removals != 2 {
		t.Fatalf("cleanup removal attempts = %d, want 2", removals)
	}
	if _, err := os.Stat(filepath.Join(cleanupPath, databaseMigrationCleanupClaimName)); err != nil {
		t.Fatalf("cleanup claim was not retained: %v", err)
	}
	if _, err := os.Stat(receiptPath); err != nil {
		t.Fatalf("migration receipt was not retained: %v", err)
	}

	databaseMigrationBeforeCleanupChildRemoval = previous
	if err := MigrateRigFromBeads(townRoot, rigName, sourcePath); err != nil {
		t.Fatalf("resume after partial cleanup: %v", err)
	}
...
func TestRemoveDatabaseRefusesRunningIsolatedServer(t *testing.T) {
	...
	createTable := exec.Command(doltPath, "sql", "-q", "CREATE TABLE payload (id INT PRIMARY KEY)")
	createTable.Dir = staged
	if output, err := createTable.CombinedOutput(); err != nil {
		t.Fatalf("creating staged user table: %v: %s", err, output)
	}
...
func TestRemoveDatabaseResumesPreparedClaim(t *testing.T) {
	...
	if err := RemoveDatabase(townRoot, "testdb_remove", true); err != nil {
		t.Fatalf("RemoveDatabase() prepared-claim resume error = %v", err)
	}
	if _, err := os.Stat(claimedPath); !os.IsNotExist(err) {
		t.Fatalf("claimed directory remains after resume: %v", err)
	}
	if _, err := os.Stat(receiptPath); !os.IsNotExist(err) {
		t.Fatalf("receipt remains after prepared-claim resume: %v", err)
	}
}
...
func TestEnsureDatabaseCleanupTargetValidatesExistingClaim(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(claimedPath, ".dolt", "noms", "manifest"), []byte("replacement"), 0o600); err != nil {
		t.Fatal(err)
	}

	err = ensureDatabaseCleanupTarget(townRoot, "testdb_remove", dbPath, want, true, false)
	if err == nil || !strings.Contains(err.Error(), "incarnation") {
		t.Fatalf("ensureDatabaseCleanupTarget() error = %v, want claimed incarnation mismatch", err)
	}
}
...
func TestRemoveDatabaseQuarantinesReferencedPendingReceipt(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "referenced") || !strings.Contains(err.Error(), "quarantined") {
		t.Fatalf("RemoveDatabase() stale receipt error = %v, want referenced quarantine", err)
	}
	if _, err := os.Stat(filepath.Join(dbPath, ".dolt")); err != nil {
		t.Fatalf("referenced replacement was mutated: %v", err)
	}
	if _, err := os.Stat(receiptPath); !os.IsNotExist(err) {
		t.Fatalf("stale receipt was not quarantined: %v", err)
	}
	quarantined, err := filepath.Glob(receiptPath + ".*.quarantine")
	if err != nil || len(quarantined) != 1 {
		t.Fatalf("quarantined receipts = %v, err = %v, want one", quarantined, err)
	}
}
...
func TestRemoveDatabaseIfCreationTokenRemovesReferencedIncompleteRig(t *testing.T) {
	...
	if err := RemoveDatabaseIfCreationToken(townRoot, "testdb_remove", token, true); err != nil {
		t.Fatalf("RemoveDatabaseIfCreationToken() error = %v", err)
	}
	if _, err := os.Stat(dbPath); !os.IsNotExist(err) {
		t.Fatalf("owned incomplete database remains: %v", err)
	}
}
...
func TestRemoveDatabaseIfCreationTokenPreservesWrongGeneration(t *testing.T) {
	...
	if err := RemoveDatabaseIfCreationToken(townRoot, "testdb_remove", "stale-generation", true); err == nil || !strings.Contains(err.Error(), "creation token") {
		t.Fatalf("RemoveDatabaseIfCreationToken() error = %v, want generation mismatch", err)
	}
	if _, err := os.Stat(filepath.Join(dbPath, ".dolt")); err != nil {
		t.Fatalf("wrong-generation cleanup mutated database: %v", err)
	}
}
...
func TestReleaseDatabaseCreationTokenIsGenerationBoundAndIdempotent(t *testing.T) {
	...
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("creation marker remains after release: %v", err)
	}
	if err := ReleaseDatabaseCreationToken(townRoot, "testdb_remove", token); err != nil {
...
func TestRemoveDatabaseQuarantinesIncarnationMismatch(t *testing.T) {
	...
	receipt := databaseCleanupReceipt{
		Version: databaseCleanupReceiptVersion, Database: "testdb_remove", Force: true,
		Phase: databaseCleanupPrepared, Incarnation: "dolt-root:vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv/manifest-sha256:9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08",
	}
	if err := writeDatabaseCleanupReceipt(receiptPath, receipt); err != nil {
		t.Fatal(err)
	}
	...
	err := RemoveDatabase(townRoot, "testdb_remove", true)
	if err == nil || !strings.Contains(err.Error(), "incarnation") || !strings.Contains(err.Error(), "quarantined") {
		t.Fatalf("RemoveDatabase() replacement error = %v, want incarnation quarantine", err)
	}
	if _, err := os.Stat(filepath.Join(dbPath, ".dolt")); err != nil {
		t.Fatalf("replacement database was mutated: %v", err)
	}
...
func TestClaimDatabaseCleanupDirectoryPreservesNameReuse(t *testing.T) {
	townRoot := t.TempDir()
	dbPath := filepath.Join(townRoot, ".dolt-data", "testdb_remove")
	manifestPath := filepath.Join(dbPath, ".dolt", "noms", "manifest")
	if err := os.MkdirAll(filepath.Dir(manifestPath), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(manifestPath, []byte("incarnation-a"), 0o600); err != nil {
		t.Fatal(err)
	}
	incarnation, err := databaseCleanupIncarnation(townRoot, "testdb_remove", dbPath, false)
	if err != nil {
	...
	if err := os.WriteFile(manifestPath, []byte("incarnation-b"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.RemoveAll(claimedPath); err != nil {
		t.Fatal(err)
	}
	if got, err := os.ReadFile(manifestPath); err != nil || string(got) != "incarnation-b" {
		t.Fatalf("replacement manifest = %q, err = %v, want incarnation-b", got, err)
	}
}

func TestRemoveDatabaseOfflineRestoresClaimAfterOwnershipRace(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	dbPath := filepath.Join(townRoot, ".dolt-data", "testdb_remove")
	manifestPath := filepath.Join(dbPath, ".dolt", "noms", "manifest")
	...
	claimDir := filepath.Dir(databaseCleanupClaimPath(townRoot, "testdb_remove", incarnation))
	...
	if _, err := os.Stat(filepath.Join(dbPath, ".dolt")); err != nil {
		t.Fatalf("database was not restored after ownership race: %v", err)
	}
...
func TestRemoveDatabaseResumesOfflineClaim(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	dbPath := filepath.Join(townRoot, ".dolt-data", "testdb_remove")
	manifestPath := filepath.Join(dbPath, ".dolt", "noms", "manifest")
	...
	receipt := databaseCleanupReceipt{
		Version: databaseCleanupReceiptVersion, Database: "testdb_remove", Force: true,
		Phase: databaseCleanupPhase("offline-prepared"), Incarnation: incarnation,
	}
	if err := writeDatabaseCleanupReceipt(receiptPath, receipt); err != nil {
		t.Fatal(err)
	}
	...
	if err := RemoveDatabase(townRoot, "testdb_remove", true); err != nil {
		t.Fatalf("RemoveDatabase() offline resume error = %v", err)
	}
	if _, err := os.Stat(claimedPath); !os.IsNotExist(err) {
		t.Fatalf("claimed directory remains after offline resume: %v", err)
	}
	if _, err := os.Stat(receiptPath); !os.IsNotExist(err) {
		t.Fatalf("receipt remains after offline resume: %v", err)
	}
}

func TestRemoveDatabaseFailsClosedWhileDoltStartLockHeld(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	dbPath := filepath.Join(townRoot, ".dolt-data", "testdb_remove")
	setupDoltDB(t, filepath.Dir(dbPath), filepath.Base(dbPath))
	lockPath := filepath.Join(filepath.Dir(DefaultConfig(townRoot).LogFile), "dolt.lock")
	if err := os.MkdirAll(filepath.Dir(lockPath), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	t.Cleanup(func() { _ = startLock.Unlock() })

	err := RemoveDatabase(townRoot, "testdb_remove", true)
	if err == nil || !strings.Contains(err.Error(), "start") {
		t.Fatalf("RemoveDatabase() error = %v, want start-lock refusal", err)
	}
	if _, err := os.Stat(filepath.Join(dbPath, ".dolt")); err != nil {
		t.Fatalf("database mutated while Dolt start lock was held: %v", err)
	}
}
...
func TestRemoveDatabaseOfflineReceiptFailurePreservesOriginal(t *testing.T) {
	t.Setenv("GT_DOLT_PORT", "1")
	townRoot := t.TempDir()
	dbPath := filepath.Join(townRoot, ".dolt-data", "testdb_remove")
	manifestPath := filepath.Join(dbPath, ".dolt", "noms", "manifest")
	...
	oldSync := databaseCleanupFileSync
	databaseCleanupFileSync = func(*os.File) error { return errors.New("sync failed") }
	t.Cleanup(func() { databaseCleanupFileSync = oldSync })

	err := RemoveDatabase(townRoot, "testdb_remove", true)
	if err == nil || !strings.Contains(err.Error(), "sync failed") {
		t.Fatalf("RemoveDatabase() error = %v, want receipt sync failure", err)
	}
	if _, err := os.Stat(filepath.Join(dbPath, ".dolt")); err != nil {
		t.Fatalf("database mutated before receipt became durable: %v", err)
	}
}

func TestPendingDatabaseCleanupNamesQuarantinesInvalidAndContinues(t *testing.T) {
	townRoot := t.TempDir()
	...
	if err := os.WriteFile(legacyPath, []byte("{\"version\":1,\"database\":\"legacy\",\"force\":true}\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	current := databaseCleanupReceipt{
		Version: databaseCleanupReceiptVersion, Database: "current", Force: true,
		Phase:       databaseCleanupPrepared,
		Incarnation: "dolt-root:0123456789abcdefghijklmnopqrstuv/manifest-sha256:9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08",
	}
	if err := writeDatabaseCleanupReceipt(filepath.Join(dir, "current.json"), current); err != nil {
		t.Fatal(err)
	}

	names, err := PendingDatabaseCleanupNames(townRoot)
	if err != nil {
		t.Fatalf("PendingDatabaseCleanupNames() error = %v", err)
	}
	if len(names) != 1 || names[0] != "current" {
		t.Fatalf("pending names = %v, want [current]", names)
	}
	if _, err := os.Stat(legacyPath); !os.IsNotExist(err) {
		t.Fatalf("legacy receipt was not quarantined: %v", err)
	}
	quarantined, err := filepath.Glob(legacyPath + ".*.quarantine")
	if err != nil || len(quarantined) != 1 {
		t.Fatalf("legacy quarantines = %v, err = %v, want one", quarantined, err)
	}
}

func TestWriteDatabaseCleanupReceiptFailsClosedOnFileSyncError(t *testing.T) {
	path := filepath.Join(t.TempDir(), "receipts", "testdb_remove.json")
	oldSync := databaseCleanupFileSync
	databaseCleanupFileSync = func(*os.File) error { return errors.New("sync failed") }
	t.Cleanup(func() { databaseCleanupFileSync = oldSync })
	receipt := databaseCleanupReceipt{
		Version: databaseCleanupReceiptVersion, Database: "testdb_remove", Force: true,
		Phase: databaseCleanupPrepared, Incarnation: "dolt-root:0123456789abcdefghijklmnopqrstuv",
	}

	if err := writeDatabaseCleanupReceipt(path, receipt); err == nil || !strings.Contains(err.Error(), "sync failed") {
		t.Fatalf("writeDatabaseCleanupReceipt() error = %v, want file sync failure", err)
	}
	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("unsynced receipt was published: %v", err)
	}
}

func TestWriteDatabaseCleanupReceiptReportsDirectorySyncError(t *testing.T) {
	path := filepath.Join(t.TempDir(), "receipts", "testdb_remove.json")
	oldSync := databaseCleanupDirSync
	databaseCleanupDirSync = func(f *os.File) error {
	...
	if err := writeDatabaseCleanupReceipt(path, receipt); err == nil || !strings.Contains(err.Error(), "directory sync failed") {
		t.Fatalf("writeDatabaseCleanupReceipt() error = %v, want directory sync failure", err)
	}
	if _, err := os.Stat(path); err != nil {
		t.Fatalf("renamed receipt missing after directory sync failure: %v", err)
	}
}
...
func TestFindOrphanedDatabasesMatchesOwnersCaseInsensitively(t *testing.T) {
	townRoot := t.TempDir()
	setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), "rigdb")
	setupRigsJSON(t, townRoot, []string{})
	setupRigMetadata(t, townRoot, "hq", "RIGDB")
	...
	if len(orphans) != 0 {
		t.Fatalf("case-only owner mismatch produced orphans: %v", orphans)
	}
}

func TestFindOrphanedDatabasesProtectsSharedDatabaseCaseInsensitively(t *testing.T) {
	townRoot := t.TempDir()
	setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), "BEADS_GLOBAL")
	setupRigsJSON(t, townRoot, []string{})
	...
	if len(orphans) != 0 {
		t.Fatalf("case-only protected database produced orphans: %v", orphans)
	}
}

func TestFindOrphanedDatabasesFailsClosedOnMalformedMetadata(t *testing.T) {
	townRoot := t.TempDir()
	setupDoltDB(t, filepath.Join(townRoot, ".dolt-data"), "rigdb")
	setupRigsJSON(t, townRoot, []string{})
	if err := os.MkdirAll(filepath.Join(townRoot, ".beads"), 0o755); err != nil {
	...
	if err := os.WriteFile(filepath.Join(townRoot, ".beads", "metadata.json"), []byte("{invalid\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	if _, err := FindOrphanedDatabases(townRoot); err == nil || !strings.Contains(err.Error(), "metadata") {
		t.Fatalf("FindOrphanedDatabases() error = %v, want malformed metadata failure", err)
	}
}
...
func TestRemoveDatabaseRefusesProtectedSharedDatabaseCaseInsensitively(t *testing.T) {
	err := RemoveDatabase(t.TempDir(), "BEADS_GLOBAL", true)
	if err == nil || !strings.Contains(err.Error(), "protected") {
		t.Fatalf("RemoveDatabase() error = %v, want case-insensitive protected refusal", err)
	}
}
...
func TestCollectDatabaseOwnersCanonicalizesCase(t *testing.T) {
	townRoot := t.TempDir()
	setupRigsJSON(t, townRoot, []string{})
	setupRigMetadata(t, townRoot, "hq", "RIGDB")

	owners := CollectDatabaseOwners(townRoot)
	if owners["rigdb"] != "town beads" {
		t.Fatalf("canonical owner = %q, want town beads", owners["rigdb"])
	}
	if _, exists := owners["RIGDB"]; exists {
		t.Fatalf("owners retained case-sensitive duplicate: %v", owners)
	}
}
