...
type HandlerResult struct {
	...
	MailSent      string // Durable Mayor notification outcome: submitted, queued, or wake-failed.
	Error         error
}
...
func HandlePolecatDone(bd *BdCli, workDir, rigName string, msg *mail.Message, router *mail.Router) *HandlerResult {
	...
	if result.Handled {
		outcome, err := notifyMayorSlotOpen(workDir, rigName, payload.PolecatName, payload.Exit, router)
		result.MailSent = string(outcome)
...
func HandlePolecatDoneFromBead(bd *BdCli, workDir, rigName, polecatName string, fields *beads.AgentFields, router *mail.Router) *HandlerResult {
	...
	if payload.PushFailed {
		...
		outcome, err := notifyMayorPushFailed(workDir, polecatName, payload.Branch, payload.IssueID, router)
		result.MailSent = string(outcome)
		result.Error = errors.Join(result.Error, err)
		return result
	}
	...
	if result.Handled {
		outcome, err := notifyMayorSlotOpen(workDir, rigName, polecatName, payload.Exit, router)
		result.MailSent = string(outcome)
		result.Error = errors.Join(result.Error, err)
	}
...
// MayorNotificationOutcome reports whether the durable notification was submitted
// to the live session or queued for later delivery.
type MayorNotificationOutcome string

const (
	MayorNotificationSubmitted  MayorNotificationOutcome = "submitted"
	MayorNotificationQueued     MayorNotificationOutcome = "queued"
	MayorNotificationWakeFailed MayorNotificationOutcome = "wake-failed"
)

var (
	mayorRouterSend = func(router *mail.Router, msg *mail.Message) error { return router.Send(msg) }
	mayorRouterWait = func(router *mail.Router) error { return router.WaitPendingNotifications() }
)

// DeliverMayorNotification is the production Witness-to-Mayor seam. It stores
// one durable high-priority record before observing the receipt-aware wake result.
func DeliverMayorNotification(router *mail.Router, subject, body string) (MayorNotificationOutcome, error) {
	if router == nil {
		return "", errors.New("Mayor notification router unavailable")
	}
	msg := mail.NewMessage("witness/", "mayor/", subject, body)
	msg.Priority = mail.PriorityHigh
	msg.Type = mail.TypeNotification
	if err := mayorRouterSend(router, msg); err != nil {
		return "", errors.New("durable Mayor notification could not be stored")
	}
	if err := mayorRouterWait(router); err != nil {
		if errors.Is(err, mail.ErrNotificationFailed) {
			return MayorNotificationWakeFailed, nil
		}
		if errors.Is(err, mail.ErrNotificationQueued) {
			return MayorNotificationQueued, nil
		}
		return MayorNotificationWakeFailed, nil
	}
	return MayorNotificationSubmitted, nil
}

func routeMayorNotification(workDir, townRoot string, router *mail.Router, subject, body string) (MayorNotificationOutcome, error) {
	if router == nil {
		router = mail.NewRouterWithTownRoot(workDir, townRoot)
	}
	return DeliverMayorNotification(router, subject, body)
}

func notifyMayorPushFailed(workDir, polecatName, branch, issueID string, router *mail.Router) (MayorNotificationOutcome, error) {
	townRoot, _ := workspace.Find(workDir)
	if townRoot == "" {
		return "", errors.New("Mayor notification workspace unavailable")
	}
	subject := fmt.Sprintf("PUSH_FAILED: %s requires recovery", polecatName)
	body := fmt.Sprintf("PUSH_FAILED: polecat=%s branch=%s issue=%s — branch not on origin, possible work loss", polecatName, branch, issueID)
	return routeMayorNotification(workDir, townRoot, router, subject, body)
}

// notifyMayorSlotOpen notifies the Mayor that a polecat slot is now open.
...
func notifyMayorSlotOpen(workDir, rigName, polecatName, exitType string, router *mail.Router) (MayorNotificationOutcome, error) {
	townRoot, _ := workspace.Find(workDir)
	if townRoot == "" {
		return "", nil
	}
	...
	if result, err := runSchedulerForSlotOpen(townRoot); err != nil {
		...
		if status, ok := schedulerOpenAfterSlot(result); ok {
			return notifyMayorSchedulerOpen(workDir, townRoot, rigName, polecatName, exitType, status, router)
		}
		return "", nil
...
func notifyMayorSchedulerOpen(workDir, townRoot, rigName, polecatName, exitType string, status slotOpenSchedulerStatus, router *mail.Router) (MayorNotificationOutcome, error) {
	...
	return routeMayorNotification(workDir, townRoot, router, subject, msg)
}
...
// RestartPolecatSession replaces only the exact session generation confirmed by
// the patrol guard. A same-name replacement is preserved, and the returned bool
// reports whether the old generation was stopped even when the new start fails.
...
func restartPolecatSessionGenerationWith(generation tmux.SessionGeneration, alive bool, stopExact func(tmux.SessionGeneration) error, start func() error) (bool, error) {
	performed := false
	if alive {
		if err := stopExact(generation); err != nil {
			return false, fmt.Errorf("stopping exact session generation: %w", err)
		}
		performed = true
	}
	if err := start(); err != nil {
		return performed, fmt.Errorf("starting replacement session: %w", err)
	}
	return true, nil
}
...
func guardZombieMutation(expectedIncarnation string, expectedSession tmux.SessionGeneration, expectedSessionAlive bool, zombie ZombieResult, ops zombieMutationGuardOps) (ZombieResult, bool) {
	...
	if disposition.Verdict != polecat.WorkstateVerdictSafeToNuke {
		if disposition.NeedsRecovery || !disposition.SafeToNuke {
			zombie.Action = "preserved-recovery-hold"
			return zombie, true
		}
		zombie.Action = "preserved-recovery-check-error"
		zombie.Error = errors.New("non-canonical recovery disposition")
		return zombie, true
	}
	if !disposition.Reusable || !disposition.SafeToNuke || disposition.NeedsRecovery || disposition.NeedsMQSubmit || disposition.CountsTowardCapacity || len(disposition.Blockers) != 0 {
		zombie.Action = "preserved-recovery-check-error"
		zombie.Error = errors.New("non-canonical recovery disposition")
		return zombie, true
	}
	...
	performed, err := ops.mutate(&zombie, current, currentSession, currentSessionAlive)
...
func guardedZombieMutation(bd *BdCli, workDir, townRoot, rigName, polecatName, sessionName string, t *tmux.Tmux, snap *agentBeadSnapshot, zombie ZombieResult, stillZombie func(*agentBeadSnapshot) (bool, error), mutate func(*ZombieResult, *agentBeadSnapshot, tmux.SessionGeneration, bool) (bool, error)) (ZombieResult, bool) {
...
func detectZombieLiveSession(bd *BdCli, workDir, townRoot, rigName, polecatName, sessionName string, t *tmux.Tmux, doneIntent *DoneIntent, witCfg *config.WitnessThresholds, snap *agentBeadSnapshot) (ZombieResult, bool) {
	...
	if doneIntent != nil && time.Since(doneIntent.Timestamp) > witCfg.DoneIntentStuckTimeoutD() {
		...
		return guardedZombieMutation(bd, workDir, townRoot, rigName, polecatName, sessionName, t, snap, zombie,
			...
			func(current *ZombieResult, _ *agentBeadSnapshot, generation tmux.SessionGeneration, alive bool) (bool, error) {
				performed, err := restartPolecatSession(workDir, rigName, polecatName, t, generation, alive)
				if err != nil {
...
func detectZombieDeadSession(bd *BdCli, workDir, townRoot, rigName, polecatName, sessionName string, t *tmux.Tmux, doneIntent *DoneIntent, detectedAt time.Time, witCfg *config.WitnessThresholds, snap *agentBeadSnapshot) (ZombieResult, bool) {
	...
	return guardedZombieMutation(bd, workDir, townRoot, rigName, polecatName, sessionName, t, snap, zombie,
		...
		func(current *ZombieResult, currentSnapshot *agentBeadSnapshot, generation tmux.SessionGeneration, alive bool) (bool, error) {
			handleZombieRestart(bd, workDir, rigName, polecatName, currentSnapshot.HookBead, currentSnapshot.cleanupStatus(), current, func() (bool, error) {
				return restartPolecatSession(workDir, rigName, polecatName, t, generation, alive)
			})
...
func handleZombieRestart(bd *BdCli, workDir, rigName, polecatName, hookBead, cleanupStatus string, zombie *ZombieResult, restart func() (bool, error)) {
	...
	performed, err := restart()
	zombie.MutationPerformed = zombie.MutationPerformed || performed
	if err != nil {
		if zombie.Error == nil {
...
func processDiscoveredCompletion(bd *BdCli, workDir, rigName string, payload *PolecatDonePayload, discovery *CompletionDiscovery, router *mail.Router) {
	...
	if payload.PushFailed {
		...
		outcome, err := notifyMayorPushFailed(workDir, payload.PolecatName, payload.Branch, payload.IssueID, router)
		if err != nil {
			discovery.Error = err
		} else {
			discovery.Action += " mayor-notification=" + string(outcome)
		}
		return
	}
